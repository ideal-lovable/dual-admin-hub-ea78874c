...
import { Mail, Zap, ClipboardList, AlertTriangle, Plus, Edit } from "lucide-react";
...
const pathToTab: Record<string, string> = {
  "/admin/automation/emails": "templates",
  "/admin/automation/triggers": "triggers",
  "/admin/automation/logs": "logs",
  "/admin/automation/failed": "failed",
};
const tabToPath: Record<string, string> = {
  templates: "/admin/automation/emails",
  triggers: "/admin/automation/triggers",
  logs: "/admin/automation/logs",
  failed: "/admin/automation/failed",
};

export default function AdminAutomation() {
  const location = useLocation();
  const navigate = useNavigate();
  const activeTab = pathToTab[location.pathname] || "templates";
  const [showTemplate, setShowTemplate] = useState(false);

  return (
    <SuperAdminLayout title="Automation & Notifications" subtitle="Manage email templates, trigger rules, and delivery logs" breadcrumbs={[{ label: "Dashboard", path: "/admin/dashboard" }, { label: "Automation" }]}>
      <Tabs value={activeTab} onValueChange={(v) => navigate(tabToPath[v])} className="space-y-4">
        ...
        <TabsContent value="templates">
          <div className="flex justify-end mb-4"><Button size="sm" className="gradient-primary text-primary-foreground gap-1.5" onClick={() => setShowTemplate(true)}><Plus className="h-3.5 w-3.5" /> New Template</Button></div>
          <div className="space-y-3">
            {emailTemplates.map(t => (
              <Card key={t.id} className="bg-card border-border">
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3"><Mail className="h-4 w-4 text-primary" /><div><p className="text-sm font-medium">{t.name}</p><p className="text-xs text-muted-foreground">Trigger: {t.trigger} • Last sent: {t.lastSent}</p></div></div>
          ...
          <div className="space-y-3">
            {triggerRules.map(r => (
              <Card key={r.id} className="bg-card border-border">
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3"><Zap className="h-4 w-4 text-accent" /><div><p className="text-sm font-medium">{r.name}</p><p className="text-xs text-muted-foreground">If: {r.condition} → Then: {r.action}</p></div></div>
        ...
        <TabsContent value="logs">
          <Card className="bg-card border-border">
            <CardContent className="p-0">
              <Table>
                <TableHeader><TableRow className="border-border"><TableHead className="text-xs">Template</TableHead><TableHead className="text-xs">Recipient</TableHead><TableHead className="text-xs">Status</TableHead><TableHead className="text-xs">Time</TableHead></TableRow></TableHeader>
                <TableBody>
                  {logs.map(l => (
                    <TableRow key={l.id} className="border-border">
                      ...
                      <TableCell><Badge variant={l.status === "delivered" ? "default" : "destructive"} className="text-xs">{l.status}</Badge></TableCell>
        ...
        <TabsContent value="failed">
          <Card className="bg-card border-border">
            <CardContent className="p-0">
              <Table>
                <TableHeader><TableRow className="border-border"><TableHead className="text-xs">Template</TableHead><TableHead className="text-xs">Recipient</TableHead><TableHead className="text-xs">Error</TableHead><TableHead className="text-xs">Time</TableHead><TableHead className="text-xs w-16"></TableHead></TableRow></TableHeader>
                <TableBody>
                  {logs.filter(l => l.status === "failed").map(l => (
                    <TableRow key={l.id} className="border-border">
                      ...
                      <TableCell className="text-xs text-muted-foreground">{l.time}</TableCell>
                      <TableCell><Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => toast.success("Retrying...")}>Retry</Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
