...
const pathToTab: Record<string, string> = {
  "/admin/discover/banner": "banners",
  "/admin/discover/watch": "watch",
  "/admin/discover/buy": "buy",
  "/admin/discover/trending": "trending",
  "/admin/discover/picks": "picks",
  "/admin/discover/events": "events",
};
const tabToPath: Record<string, string> = {
  banners: "/admin/discover/banner",
  watch: "/admin/discover/watch",
  buy: "/admin/discover/buy",
  trending: "/admin/discover/trending",
  picks: "/admin/discover/picks",
  events: "/admin/discover/events",
};

export default function AdminDiscover() {
  const location = useLocation();
  const navigate = useNavigate();
  const activeTab = pathToTab[location.pathname] || "banners";
  const [showAdd, setShowAdd] = useState(false);

  return (
    <SuperAdminLayout title="Discover & Curation" subtitle="Manage featured content, banners, and trending sections" breadcrumbs={[{ label: "Dashboard", path: "/admin/dashboard" }, { label: "Discover & Curation" }]}>
      <Tabs value={activeTab} onValueChange={(v) => navigate(tabToPath[v])} className="space-y-4">
        ...
        <TabsContent value="banners">
          <div className="flex justify-end mb-4"><Button size="sm" className="gradient-primary text-primary-foreground gap-1.5" onClick={() => setShowAdd(true)}><Plus className="h-3.5 w-3.5" /> Add Banner</Button></div>
              ...
              <Card key={b.id} className="bg-card border-border">
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="h-10 w-16 rounded bg-primary/10 flex items-center justify-center"><Megaphone className="h-4 w-4 text-primary" /></div>
                    <div><p className="text-sm font-medium">{b.title}</p><p className="text-xs text-muted-foreground">Position: {b.position}</p></div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant={b.status === "active" ? "default" : "secondary"} className="text-xs">{b.status}</Badge>
                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => toast.success("Editing")}><Edit className="h-3.5 w-3.5" /></Button>
                    <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" onClick={() => toast.success("Removed")}><Trash2 className="h-3.5 w-3.5" /></Button>
                  </div>
                </CardContent>
              </Card>
            ))}
            ...
            <CardContent className="space-y-3">
              {trendingItems.filter(i => i.type === "video").map(item => (
                <div key={item.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                  <div className="flex items-center gap-3"><Video className="h-4 w-4 text-primary" /><div><p className="text-sm font-medium">{item.title}</p><p className="text-xs text-muted-foreground">by {item.creator} • {item.views} views</p></div></div>
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent className="space-y-3">
              {trendingItems.filter(i => i.type === "product").map(item => (
                <div key={item.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                  <div className="flex items-center gap-3"><ShoppingCart className="h-4 w-4 text-accent" /><div><p className="text-sm font-medium">{item.title}</p><p className="text-xs text-muted-foreground">by {item.creator} • {item.views} views</p></div></div>
                  <div className="flex gap-1">
                    <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => toast.success("Featured!")}>Feature</Button>
                    <Button size="sm" variant="ghost" className="h-7 text-xs text-destructive" onClick={() => toast.success("Removed")}>Remove</Button>
                  </div>
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent className="space-y-3">
              {trendingItems.map((item, i) => (
                <div key={item.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                  <div className="flex items-center gap-3"><span className="text-lg font-bold text-primary w-6">#{i + 1}</span><div><p className="text-sm font-medium">{item.title}</p><p className="text-xs text-muted-foreground">{item.views} views</p></div></div>
            ...
            <CardContent className="space-y-3">
              {creatorPicks.map(pick => (
                <div key={pick.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                  <div className="flex items-center gap-3"><Star className="h-4 w-4 text-yellow-400" /><div><p className="text-sm font-medium">{pick.creator} → {pick.product}</p><p className="text-xs text-muted-foreground">Store: {pick.store}</p></div></div>
        ...
        <TabsContent value="events">
          <Card className="bg-card border-border">
            <CardContent className="p-8 text-center">
              ...
              <Button size="sm" className="mt-3 gradient-primary text-primary-foreground" onClick={() => setShowAdd(true)}><Plus className="h-3.5 w-3.5 mr-1" /> Create Event</Button>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
