...
const pathToTab: Record<string, string> = {
  ...
  "/admin/config/metadata": "metadata",
  "/admin/config/tags": "tags",
  "/admin/config/homepage": "homepage",
  "/admin/config/discover": "discover",
};
const tabToPath: Record<string, string> = {
  categories: "/admin/config/categories",
  metadata: "/admin/config/metadata",
  tags: "/admin/config/tags",
  homepage: "/admin/config/homepage",
  discover: "/admin/config/discover",
};

export default function AdminPlatformConfig() {
  const location = useLocation();
  const navigate = useNavigate();
  const activeTab = pathToTab[location.pathname] || "categories";
  const [showAdd, setShowAdd] = useState(false);

  return (
    <SuperAdminLayout title="Platform Configuration" subtitle="Manage categories, tags, metadata, and layout controls" breadcrumbs={[{ label: "Dashboard", path: "/admin/dashboard" }, { label: "Platform Config" }]}>
      <Tabs value={activeTab} onValueChange={(v) => navigate(tabToPath[v])} className="space-y-4">
        <TabsList className="bg-secondary border border-border">
          ...
          <TabsTrigger value="homepage" className="gap-1.5"><LayoutGrid className="h-3.5 w-3.5" /> Homepage</TabsTrigger>
          <TabsTrigger value="discover" className="gap-1.5"><Eye className="h-3.5 w-3.5" /> Discover</TabsTrigger>
        </TabsList>

        <TabsContent value="categories">
          <div className="flex justify-end mb-4"><Button size="sm" className="gradient-primary text-primary-foreground gap-1.5" onClick={() => setShowAdd(true)}><Plus className="h-3.5 w-3.5" /> Add Category</Button></div>
          <div className="grid gap-3 md:grid-cols-2">
            {categories.map(c => (
              <Card key={c.id} className="bg-card border-border">
                <CardContent className="p-4 flex items-center justify-between">
                  <div><p className="text-sm font-medium">{c.name}</p><p className="text-xs text-muted-foreground">/{c.slug} • {c.items.toLocaleString()} items</p></div>
          ...
          <Card className="bg-card border-border">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" className="gradient-primary text-primary-foreground gap-1.5" onClick={() => setShowAdd(true)}><Plus className="h-3.5 w-3.5" /> Add Tag</Button>
              </div>
            </CardHeader>
            <CardContent>
            ...
            <CardContent className="space-y-2">
              {homepageSections.map(s => (
                <div key={s.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary/30 hover:bg-secondary/50 transition-colors">
                  <div className="flex items-center gap-3"><GripVertical className="h-4 w-4 text-muted-foreground cursor-grab" /><span className="text-sm font-medium">{s.name}</span></div>
                  <div className="flex items-center gap-2">
                    <Badge variant={s.visible ? "default" : "secondary"} className="text-xs">{s.visible ? "Visible" : "Hidden"}</Badge>
                    <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => toast.success(`${s.name} toggled`)}>{s.visible ? "Hide" : "Show"}</Button>
                  </div>
                </div>
              ))}
            </CardContent>
