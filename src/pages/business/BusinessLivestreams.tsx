...
export default function BusinessLivestreams() {
  ...
  const handleGoLive = () => {
    if (selectedStream) { setStreams(streams.map(s => s.id === selectedStream.id ? { ...s, status: "live" as const } : s)); setIsGoLiveOpen(false); toast.success("You're now live! 🔴"); }
  };
        ...
        <div className="flex gap-2">
          {isScheduled ? (
            <>
              ...
              <Button size="sm" className="flex-1 gradient-primary text-primary-foreground" onClick={() => { setSelectedStream(stream); setIsGoLiveOpen(true); }}><Radio className="h-4 w-4 mr-1" /> Go Live</Button>
            </>
          ) : (
            <>
  ...
  return (
    <AdminLayout type="business" title="Livestreams" subtitle="Manage live shopping events">
      ...
      <Dialog open={isGoLiveOpen} onOpenChange={setIsGoLiveOpen}>
        <DialogContent className="bg-card border-border">
          <DialogHeader><DialogTitle className="font-display">Go Live Now?</DialogTitle></DialogHeader>
          <p className="text-sm text-muted-foreground py-4">Start your live shopping event "<span className="text-foreground font-medium">{selectedStream?.title}</span>" with {selectedStream?.products} products?</p>
          <DialogFooter><DialogClose asChild><Button variant="secondary">Not Yet</Button></DialogClose><Button className="gradient-primary text-primary-foreground gap-2" onClick={handleGoLive}><Radio className="h-4 w-4" /> Go Live</Button></DialogFooter>
        </DialogContent>
      </Dialog>
