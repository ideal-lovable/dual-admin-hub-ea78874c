...
export default function BusinessMessages() {
  ...
  return (
    <AdminLayout type="business" title="Messages" subtitle="Customer conversations">
          ...
          <div className="flex h-full">
            ...
            <div className="w-80 border-r border-border flex flex-col">
              <div className="p-4 border-b border-border">
                <div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input placeholder="Search conversations..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 bg-secondary border-border" /></div>
              </div>
              <div className="flex-1 overflow-y-auto">
                {filteredConvs.map((conv) => (
                  <div key={conv.id} className={`flex items-center gap-3 p-4 cursor-pointer transition-colors hover:bg-secondary/50 ${conv.id === activeConv ? "bg-secondary/80" : ""}`}
                    onClick={() => { setActiveConv(conv.id); setConversations(conversations.map(c => c.id === conv.id ? { ...c, unread: 0 } : c)); }}>
                    ...
                    {conv.unread > 0 && <Badge className="bg-primary text-primary-foreground h-5 w-5 flex items-center justify-center p-0 text-xs">{conv.unread}</Badge>}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex items-center justify-between p-4 border-b border-border">
                <div className="flex items-center gap-3">
                  ...
                  <div><h3 className="font-semibold text-foreground">{activeConversation?.name}</h3><span className={`text-xs ${activeConversation?.status === "online" ? "text-success" : "text-muted-foreground"}`}>{activeConversation?.status === "online" ? "Online" : "Offline"}</span></div>
                </div>
                <DropdownMenu>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.sender === "business" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 ${msg.sender === "business" ? "gradient-primary text-primary-foreground" : "bg-secondary text-foreground"}`}>
              ...
              <div className="p-4 border-t border-border">
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="icon" className="text-muted-foreground" onClick={() => toast.info("File attachment dialog would open here")}><Paperclip className="h-5 w-5" /></Button>
                  ...
                  <Button size="icon" className="gradient-primary text-primary-foreground" onClick={handleSend} disabled={!newMessage.trim()}><Send className="h-4 w-4" /></Button>
                </div>
              </div>
            </div>
          </div>
