...
import cinemaBanner from "@/assets/restored/cinema-banner.jpg";
...
interface ContentItem {
  id: number; title: string; type: "Movie" | "TV Show" | "Music" | "Podcast" | "Game"; genre: string; submitted: string; views: string; status: "active" | "pending" | "draft"; image: string;
}

const initialContent: ContentItem[] = [
  { id: 1, title: "The Lodge - Season 2", type: "TV Show", genre: "Thriller", submitted: "Jan 12, 2026", views: "12.4K", status: "active", image: theaterSeats },
  { id: 2, title: "Crossfire", type: "Movie", genre: "Action", submitted: "Feb 1, 2026", views: "—", status: "pending", image: cinemaBanner },
  { id: 3, title: "Summer Beats Vol. 3", type: "Music", genre: "Pop/R&B", submitted: "Jan 28, 2026", views: "8.2K", status: "active", image: cassetteImage },
  { id: 4, title: "Tomorrow", type: "Movie", genre: "Drama", submitted: "Feb 10, 2026", views: "—", status: "pending", image: projectorImage },
  { id: 5, title: "Murder at Castlewick", type: "TV Show", genre: "Mystery", submitted: "Dec 5, 2025", views: "22.1K", status: "active", image: cinemaBanner },
  { id: 6, title: "Neon Nights EP", type: "Music", genre: "Electronic", submitted: "Feb 15, 2026", views: "—", status: "draft", image: djConsole },
  { id: 7, title: "Arena Clash", type: "Game", genre: "Action RPG", submitted: "Jan 20, 2026", views: "5.6K", status: "active", image: projectorImage },
];
...
export default function PublisherContent() {
  ...
  const handleSubmit = () => {
    const item: ContentItem = { id: Date.now(), title: newItem.title, type: newItem.type, genre: newItem.genre, submitted: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }), views: "—", status: "pending", image: cinemaBanner };
  ...
  const columns = [
    { key: "content", label: "Content", render: (item: ContentItem) => {
      ...
      return (<div className="flex items-center gap-3"><div className="relative h-11 w-16 overflow-hidden rounded-lg border border-border bg-secondary"><img src={item.image} alt={item.title} className="h-full w-full object-cover" /><div className="absolute bottom-1 right-1 rounded bg-background/80 p-0.5"><Icon className="h-3 w-3 text-accent" /></div></div><div><p className="font-medium text-foreground">{item.title}</p><p className="text-sm text-muted-foreground">{item.type} · {item.genre}</p></div></div>);
    }},
    ...
    <AdminLayout type="publisher" title="Content" subtitle="Manage your submitted content">
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="bg-card border-border">
          ...
          {selectedItem && (
            <div className="space-y-4 py-4">
              <div className="h-40 overflow-hidden rounded-xl border border-border bg-secondary"><img src={selectedItem.image} alt={selectedItem.title} className="h-full w-full object-cover" /></div>
