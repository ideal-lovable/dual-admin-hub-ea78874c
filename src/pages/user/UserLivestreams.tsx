...
export default function UserLivestreams() {
  ...
  const handleGoLive = () => {
    if (selectedStream) {
      setStreams(streams.map(s => s.id === selectedStream.id ? { ...s, status: "live" as const } : s));
    ...
    <AdminLayout type="user" title="Livestreams" subtitle="Manage your live broadcasts">
      ...
      <Dialog open={isGoLiveOpen} onOpenChange={setIsGoLiveOpen}>
        <DialogContent className="bg-card border-border">
          ...
          <p className="text-sm text-muted-foreground py-4">You're about to start "<span className="text-foreground font-medium">{selectedStream?.title}</span>". Your followers will be notified. Make sure your camera and microphone are ready!</p>
          <DialogFooter>
            <DialogClose asChild><Button variant="secondary">Not Yet</Button></DialogClose>
            <Button className="gradient-primary text-primary-foreground gap-2" onClick={handleGoLive}><Radio className="h-4 w-4" /> Go Live</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
